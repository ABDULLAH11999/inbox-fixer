...
const FALLBACK_PLANS = [
  {
    ...
    scans_limit: 3,
    enabled: true,
    features: ['3 Scans/day', 'Plain English Explanations', 'DNS Fix Templates'],
  },
  {
    id: 'free',
    name: 'Free Account',
    ...
    scans_limit: 10,
    enabled: true,
    features: ['10 Scans/day', 'Plain English Explanations', 'Scan History Log'],
  },
  {
    id: 'pro',
    name: 'Pro Plan',
    ...
    scans_limit: 999999,
    enabled: true,
    features: ['Unlimited Scans', 'Daily Monitored Alerts', 'PDF Report Export', 'Priority Support'],
  },
];

function FeatureList({
  features,
  highlightUnlimited = false,
}: {
  features: string[];
  highlightUnlimited?: boolean;
}) {
  return (
    <ul className="space-y-3.5 text-xs text-[#8b9fc0]">
      {features.map((feature) => (
        <li key={feature} className="flex items-center gap-2">
          ...
          <span className={highlightUnlimited && feature.toLowerCase().includes('unlimited') ? 'text-white font-semibold' : ''}>
            {feature}
          </span>
        </li>
      ))}
...
export default function PricingPage() {
  ...
  const guestPlan = plans.find((plan) => plan.id === 'guest' && plan.enabled !== false);
  const freePlan = plans.find((plan) => plan.id === 'free' && plan.enabled !== false);
  const proPlan = plans.find((plan) => plan.id === 'pro' && plan.enabled !== false);
      ...
      <main className="flex-1 max-w-5xl mx-auto px-6 py-16 space-y-12">
        <div className="text-center space-y-4 max-w-2xl mx-auto">
          ...
          <p className="text-[#6b7fa8] text-sm md:text-base leading-relaxed">
            Keep your business domain authentic, avoid spam filters, and unlock automated daily blacklist monitoring.
            The admin can hide any plan at any time, and disabled plans will disappear here automatically.
          </p>
        </div>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <h3 className="font-syne font-bold text-2xl text-white">
                    {guestPlan.name === 'Guest' ? 'No Signup' : guestPlan.name}
                  </h3>
                ...
                <div className="py-2 border-y border-[#1e2d4a]/40">
                  <span className="font-syne text-4xl font-bold text-white">${guestPlan.price ?? 0}</span>
                  <span className="text-xs text-[#6b7fa8] font-mono"> / forever</span>
                </div>

                <FeatureList features={guestPlan.features || []} />
            ...
            <div className="bg-[#0f1729]/50 border border-[#1e2d4a]/60 rounded-3xl p-6 flex flex-col justify-between relative">
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <h3 className="font-syne font-bold text-2xl text-white">{freePlan.name}</h3>
                  <p className="text-xs text-[#6b7fa8] leading-relaxed">Perfect for small shop owners and individual marketers.</p>
                </div>

                <div className="py-2 border-y border-[#1e2d4a]/40">
                  <span className="font-syne text-4xl font-bold text-white">${freePlan.price ?? 0}</span>
            ...
            <div className="bg-[#0f1729] border-2 border-[#00ff88] rounded-3xl p-6 flex flex-col justify-between relative shadow-2xl">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <h3 className="font-syne font-bold text-2xl text-white">{proPlan.name}</h3>
                  <p className="text-xs text-[#6b7fa8] leading-relaxed">Built for freelancers, agencies, and online brand owners.</p>
                </div>

                <div className="py-2 border-y border-[#1e2d4a]/40">
                  <span className="font-syne text-4xl font-bold text-white">${proPlan.price ?? 0}</span>
                  <span className="text-xs text-[#6b7fa8] font-mono"> / month</span>
                </div>

                <FeatureList features={proPlan.features || []} highlightUnlimited />
              </div>
              ...
              <div className="pt-8">
                <button
                  ...
                  disabled={loading}
                  className="w-full bg-[#00ff88] text-[#0a0f1e] hover:bg-[#00dd77] border border-[#00ff88] px-4 py-3 rounded-xl text-xs font-syne font-bold transition-all cursor-pointer active:scale-[0.98] disabled:opacity-60"
                >
                  {loading ? 'Initiating Checkout...' : `Upgrade to ${proPlan.name} — $${proPlan.price ?? 0}/mo`}
                </button>
              </div>
            </div>
          )}
        ...
        {!guestPlan && !freePlan && !proPlan && (
          <div className="text-center text-sm text-[#6b7fa8]">
            No plans are currently available. Please contact the administrator.
          </div>
        )}
      </main>
